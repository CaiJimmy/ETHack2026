...
function EvidenceDrawer({item,onClose,onAsk}){
  ...
  return <dialog className="evidence-dialog" ref={dialog} onCancel={onClose} onClick={event=>{if(event.target===dialog.current)onClose();}} aria-labelledby="evidence-title"><div className="drawer-content">
    ...
    {plume?<>
      ...
      <button className="secondary full" aria-expanded={showScore} aria-controls="score-explanation" onClick={()=>setShowScore(v=>!v)}><Info size={16}/>{showScore?'Hide score explanation':'Explain company score'}</button>
      {showScore&&<section id="score-explanation" className="score-explanation" aria-label="Company score explanation">
        ...
        <ol className="score-method">
          ...
          <li><strong>Promise versus progress</strong><p>Measured annual change minus promised annual change. A positive gap means slower cuts than promised.</p></li>
          <li><strong>Carbon-price exposure</strong><p>Modeled carbon costs relative to operating income. Results depend on the scenario and its assumptions.</p></li>
          <li><strong>Environmental conduct</strong><p>Historical environmental enforcement penalties relative to revenue. This measures the recorded compliance history.</p></li>
        </ol>
        ...
        <div className="notice"><Info size={16}/><p>Missing evidence is not zero emissions or proof of good performance. The detail API provides the final percentile and rank interval, but not exact per-area score contributions; those contributions are not reconstructed here.</p></div>
